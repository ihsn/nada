 ...
 * Supports:
 * - [{ property, message }, ...]  (schema validator)
 * - { field: "msg", ... } / { field: ["msg", ...] }
 * - ["msg", ...]
...
function issueSuffix(count, labels) {
  const word = count === 1 ? 'issue' : 'issues';
  return ` (${count} ${word})`;
}

/**
 * Extract message + validation errors from axios / thrown API errors.
 * @param {unknown} err
...
export function extractApiError(err, labels = {}) {
  ...
  const schemaValidationFailed = labels.schemaValidationFailed || 'Schema validation failed';
  const saveFailed = labels.saveFailed || 'Save failed';

  // Avoid showing axios's generic "Request failed with status code 400" when we have details
  ...
  if (displayMessage === 'VALIDATION_ERROR' || displayMessage === 'VALIDATION_ERRORS') {
    displayMessage = errors.length
      ? validationFailed + issueSuffix(errors.length, labels)
      : validationFailed;
  }
