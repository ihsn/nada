  ...
  <div>
    ...
    <div class="catalog-page-header mb-4">
      <div class="catalog-page-header__inner">
        <h1 class="text-h5 font-weight-semibold text-high-emphasis mb-0 catalog-page-header__title">
          {{ t('dd_active_tasks', 'Active tasks') }}
        </h1>
      </div>
    </div>
    ...
    <v-card class="admin-catalog-surface mb-5" rounded="lg" elevation="1">
      <v-table density="comfortable" class="text-body-2">
        ...
        <tbody>
          <tr v-for="task in pending" :key="task.id">
            <td>
              <router-link
          ...
          <tr v-if="!loading && !pending.length">
    ...
    <h2 class="text-h6 font-weight-semibold text-high-emphasis mb-3">
      {{ t('dd_recently_completed', 'Recently completed tasks') }}
    </h2>
...
defineOptions({ name: 'AdminDepositTasksPage' });

const { t } = useI18n();
const { siteUrl } = useAppConfig();
const { fetchTasks } = useAdminDepositApi();
...
const breadcrumbItems = computed(() => [
  ...
  { title: t('dd_active_tasks', 'Active tasks'), disabled: true },
]);
...
async function load() {
  ...
  try {
    ...
    pending.value = Array.isArray(result?.pending) ? result.pending : [];
    completed.value = Array.isArray(result?.completed) ? result.completed : [];
  } catch (e) {
    pending.value = [];
    completed.value = [];
