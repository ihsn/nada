...
/**
 * Build same-origin URL for resource file download (attachment).
 * @param {string} siteUrl
 * @param {{ source?: string, sid?: number|null, idno?: string, resourceId: number }} opts
 * @returns {string}
 */
export function buildResourceDownloadUrl(siteUrl, opts) {
  const source = (opts.source || 'resource').toLowerCase();
  const resourceId = Number(opts.resourceId);
  ...
  if (opts.sid) {
    return joinSiteUrl(siteUrl, `catalog/${opts.sid}/download/${resourceId}`);
  }
