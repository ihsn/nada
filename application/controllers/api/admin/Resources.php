...
/**
 * Admin Resources API — copy of application/controllers/api/Resources.php, routed under /api/admin/resources/...
 * Original api/datasets/{id}/resources → api/resources endpoints unchanged.
 *
 * Fix broken links: PUT or POST …/resources/fix_links or …/resources/{rid}/fix_links (study-wide repair; rid is ignored).
 *
 * Access: authenticated users with {@see MY_REST_Controller::has_dataset_access} on the study (same bar as catalog edit),
 * not Ion Auth admin-only.
 */
...
class Resources extends MY_REST_Controller
{
	 ...
	 *
	 * Query parameters:
	 * - `dctype` — optional; if set, filters via Survey_resource_model::get_resources_by_type. If omitted, all resources.
	 ...
	 *
	 * Each row includes `file_exists` (local file present or filename is http/https/ftp URL) and `dataset_idno`.
	 */
	...
	{	
		try{
			...
			if($resources){
				$resources=$this->Survey_resource_model->generate_api_download_link($resources, true);
			}
	...
	function single_get($idno=null,$resource_id=null)
	{
		try{
			...
			$resources=$this->Survey_resource_model->generate_api_download_link(array($resource), true);
	...
	/**
	 * Export external resources as RDF/XML (same payload as admin/catalog/export_rdf/{sid}).
	 *
	 * GET /api/admin/resources/{studyRef}/resources/export_rdf
	 * Query `id_format=id` when `{studyRef}` is numeric surveys.id.
	 */
	public function export_rdf_get($idno = null)
	{
		...
		try {
			...
			$this->load->model('Catalog_model');
			$data = $this->Catalog_model->get_survey_rdf($sid);
			force_download('rdf-' . $sid . '.rdf', $data);
		}
		catch (Exception $e) {
			$output = array(
				'status'  => 'error',
	...
	/**
	 * POST alias for {@see fix_links_put()} when PUT cannot be sent (study-wide link repair; optional …/resources/{rid}/… URL ignores rid).
	 */
