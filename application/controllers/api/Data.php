...
class Data extends MY_REST_Controller
{
	 ...
	 * 
	 **/
	function index_get($dataset_idno=null, $fid=null)
	{
		try{
			$sid=$this->get_sid_from_idno($dataset_idno);			
			$files=$this->Data_file_resources_model->get_all_files_resources($sid,$fid);
	 ...
	 * 
	 */
	function download_get($dataset_idno=null,$fid=null)
	{
		...
		try{
			...
			if(!$fid || trim($fid)==""){
				throw new Exception("PARAM_NOT_SET: fid");
			}
echo "X";
			return $this->Data_file_resources_model->download_microdata_file($sid, $fid);
		}
		catch(Exception $e){
			$output=array(
				'status'=>'error',
