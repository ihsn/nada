...
class Access_licensed extends MY_Controller {
	...
	*/
	function download($request_id=NULL,$file_id=NULL)
	{
		...
		{	
			return $this->expired($request_id,$download_limit);
		}
		...
		if (time() > $expiry) {
			return $this->expired_date($request_id,$expiry);
		}
	...
	{	
		...
		$this->form_validation->set_rules('datause', t('intended_use_of_data'), 'trim|required|xss_clean|max_length[3000]');
	...
	**/
	private function expired($requestid=NULL,$download_limit=0)
	{
	...
	private function expired_date($requestid=NULL, $expiry=0)
	{
