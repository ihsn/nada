	...
	*/
	function _build_study_query()
	{
		...
		{		
			...
			return sprintf('MATCH(%s) AGAINST(%s IN BOOLEAN MODE)',$study_fulltext_index,$this->ci->db->escape($study_keywords));
		}
		else if(strlen($study_keywords)==3)
		{
			...
			return sprintf('%s REGEXP (%s)','surveys.titl',$this->ci->db->escape($study_keywords));	
		}
		else
		{
			return FALSE;
