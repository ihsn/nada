...
<?php $tr_class=""; ?>
	...
	<?php foreach($variables as $row):?>
    	...
    	<tr  class="<?php echo $tr_class; ?>">
            ...
            <td><?php echo anchor('ddibrowser/'.$surveyid.'/variable/'.$row['varID'],$row['name'],array('target'=>'blank_','title'=>$row['labl']));?></td>
            <td><?php echo $row['labl']?></td>
            <td><?php echo anchor('ddibrowser/'.$surveyid.'/variable/'.$row['varID'],'<img src="images/icon_question.gif" border="0"/>',array('target'=>'blank_','title'=>$row['labl']));?></td>
        </tr>
    <?php endforeach;?>
	</table>
<?php else: ?>
