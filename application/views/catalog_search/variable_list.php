...
<?php $tr_class=""; ?>
	...
	<?php foreach($rows as $row):?>
    	...
    	<tr  class="<?php echo $tr_class; ?>" valign="top">
            ...
            <td><?php echo anchor('ddibrowser/'.$row['surveyid_FK'].'/variable/'.$row['varID'],$row['name'],array('target'=>'blank_','class'=>'dlg','title'=>t('variable_info')));?></td>
            <td>
				<div class="labl" style="font-size:1.2em;"><?php echo ($row['labl']!=='') ? $row['labl'] : $row['name']; ?></div>
				<div style="color:#666666"><?php echo $row['nation']. ' - '.$row['titl']; ?></div>
            </td>
            <td><?php echo anchor('ddibrowser/'.$row['surveyid_FK'].'/variable/'.$row['varID'],'<img src="images/icon_question.gif" border="0"/>',array('target'=>'blank_','class'=>'dlg','title'=>$row['labl']));?></td>
        </tr>
    <?php endforeach;?>
	</table>
