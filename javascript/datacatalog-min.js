...
return false;})
$(".catalog-sort-links a").unbind('click').click(function(event){formdata=$(this).attr("href")+'&ajax=1&'+$("#search_form").serialize();$('#surveys').html('<img src="images/loading.gif" align="bottom" border="0"/> '+i18n.searching).load(formdata,function(data){bindBehaviors(this);});return false;});$('.accesspolicy').unbind('click').click(function(event){iframe_dialog($(this));return false;});$('.accessform').unbind('click').click(function(event){iframe_dialog($(this),on_lightbox_unload);return false;});$(".dlg").unbind('click').click(function(event){item_link=$(this).clone().attr("href",$(this).attr("href")+'?ajax=true&css=true');;iframe_dialog(item_link);return false;});variable_compare_handlers();$(".switch-page-size .button").unbind('click').click(function(event){$("#ps").val($(this).html());$("#page").val(1);advanced_search();});}
...
{selected_topics=$("#search_form .chk-topic:checked").length;if(selected_topics==0)
{$("#countries-list input:checkbox").attr('disabled',false).parent().removeClass("disabled");advanced_search();advanced_search();return;}
