...
// This file lives at <base>/api-documentation/catalog-admin/openapi.php
$script_path = dirname($_SERVER['SCRIPT_NAME']); // e.g. /NADA/api-documentation/catalog-admin
$parts = array_filter(explode('/', $script_path));
// Remove the last two segments (catalog-admin, api-documentation)
if (count($parts) >= 2) {
    array_pop($parts);
    array_pop($parts);
}
...
// Matches bare refs ('survey-schema.json'), parent-dir refs ('../schemas/citation-schema.json'),
